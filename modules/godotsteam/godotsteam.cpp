...
Image Steam::drawAvatar(int size, uint8 *buffer) {
	...
	// TODO: Does this need tweaked? Check.
	//Image avatar(size, size, false, Image::FORMAT_RGBA8);
	PoolColorArray NewPool;
	//avatar.lock();

	// Filling each pixel in the image with a color
	for (int y = 0; y < size; y++) {
		for (int x = 0; x < size; x++) {
			...
			//TemporaryAvatar->set_pixel(x, y, Color(r, g, b, a));
			NewPool.append(Color(r, g, b, a));
		}
	}

	//TemporaryAvatar->create(size, size, false, Image::FORMAT_RGBA8, PoolByteArray(Array(NewPool)));
...
void Steam::_avatar_loaded(AvatarImageLoaded_t *avatarData) {
	...
	uint32_t size = avatarData->m_iWide;
	// Get image buffer
	...
	if (TemporaryAvatar.is_valid())
	{
		TemporaryAvatar->create(size, size, false, Image::FORMAT_RGBA8, temppool);

	//printf(String(TemporaryAvatar->get_size()).utf8());

		tempf.instance();
		tempf->create_from_image(TemporaryAvatar);
	}
	...
	//drawAvatar(size, buffer);

	//Image avatar = drawAvatar(size, buffer);
	//TemporaryAvatar.copy_internals_from(&avatar);
	call_deferred("emit_signal", "avatar_loaded", avatarSize, tempf);
	//call_deferred("emit_signal", "avatar_loaded", avatarSize);
}

Ref<ImageTexture> Steam::GetLoadedAvatar() const {
	return tempf;
}
...
void Steam::_bind_methods() {
	...
	ClassDB::bind_method("GetLoadedAvatar", &Steam::GetLoadedAvatar);
